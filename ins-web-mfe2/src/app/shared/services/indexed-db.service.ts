...
export class IndexedDbService {
  ...
  // Method use to get insurance details by policy number
  public updateInsuranceDetails(policyNumber: string): Promise<any> {
    return new Promise((resolve, reject) => {
      if (!this.database) {
        reject('Database is not initialized');
        return;
      ...
      const transaction = this.database.transaction(['insuranceDetails'], 'readwrite');
      const store:any = transaction.objectStore('insuranceDetails');
      ...
      request.onsuccess = (event:any) => {
        ...
        updateRequest.onsuccess = () => {
            console.log(`Policy updated, status: ${updateRequest.result}`)
